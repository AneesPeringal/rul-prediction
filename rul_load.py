...
for alpha in list_alpha:
    ...
    model.load_state_dict(torch.load(f'saved_models\\LSTM_{alpha}'))
    def validation():
    ...
    file.write(f'Overestimation rate: {torch.dot(y - y_pred, 1.0*(y_pred<y)).item()/len(y)}')
    file.close()